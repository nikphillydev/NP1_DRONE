...
PMW3901::PMW3901(SPI_HandleTypeDef* spi_handle, osMutexId_t& spi_mutex, GPIO_TypeDef* cs_port, uint16_t cs_pin,
	...
	: spi_handle(spi_handle),
	  ...
	  rate_x_filter(MOVING_AVG_FLT_COUNT),
	  rate_y_filter(MOVING_AVG_FLT_COUNT),
	  logger(logger) {}
...
bool PMW3901::poll(float deltatime_s)
{
	...
	int16_t delta_x_raw = (rx_data[2] << 8) | rx_data[1];
	int16_t delta_y_raw = (rx_data[4] << 8) | rx_data[3];

	// Check surface quality or outliers
	uint8_t squal = rx_data[5];
	if (squal == 0 || std::abs(delta_x_raw) > 240 || std::abs(delta_y_raw) > 240)
	{
		// Bad data, reset readings
		delta_x_raw = 0;
		delta_y_raw = 0;
	}

	float delta_x = delta_x_raw / 385.0f;	// convert pixels to radians
	float delta_y = delta_y_raw / 385.0f;

	np::lock_guard lock(data_mutex);
	flow_rate[0] = rate_x_filter.update(delta_x / deltatime_s);		// convert to rad/s
	flow_rate[1] = rate_y_filter.update(delta_y / deltatime_s);

	return status;
}

std::array<float, 2> PMW3901::get_flow_rate()
{
	np::lock_guard lock(data_mutex);
	return flow_rate;
}
