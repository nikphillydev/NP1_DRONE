	...
	if (imu_init && baro_init && mag_init && flow_init)
	{
		...
		while (1)
		{
			...
			else
			{
				...
				// Fuse altitude via complimentary filter (if range finder distance valid)
				if (rf_distance_BODY < MAX_DISTANCE_M) {
					// Range finder NOT saturated
					rf_saturated = false;
					altitude = ALTITUDE_ALPHA * barometer_altitude + (1.0f - ALTITUDE_ALPHA) * rf_distance_WORLD;
				}
				else {
					// Range finder saturated, ignore
					rf_saturated = true;
					altitude = barometer_altitude;
				}

				// Log range finder saturation
				if (counter % FUSION_FREQ == 0 && rf_saturated) {
